...
class MainActivity : Activity() {
    ...
    private fun showPage(index: Int) {
        ...
        body.removeAllViews()
        body.addView(when (index) { 0 -> home(); 1 -> stats(); else -> settings() })
    }
    ...
    private fun home(): View {
        ...
        val words = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; gravity = Gravity.CENTER_VERTICAL }
        ...
        }, LinearLayout.LayoutParams(0, dp(350), 1f))
        col.addView(hero, fullWrapMargins(0, 0, 0, 12))

        val progress = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; setBackgroundColor(line) }
    ...
    private fun stats(): View {
        val scroll = ScrollView(this)
        val col = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(16), dp(2), dp(16), dp(18)) }
        val hero = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; background = cardBg(); setPadding(dp(20), dp(16), dp(8), dp(12)) }
        ...
        hero.addView(ImageView(this).apply { setImageResource(R.drawable.cheer_full); scaleType = ImageView.ScaleType.FIT_CENTER }, LinearLayout.LayoutParams(0, dp(260), .9f))
        col.addView(hero, fullWrapMargins(0, 0, 0, 12))
        listOf("실사용" to "5시간 24분", "자리비움" to "48분", "자리비움 횟수" to "6회", "최장 연속 사용" to "1시간 18분", "실사용률" to "87%").forEachIndexed { i, item ->
            col.addView(statRow(item.first, item.second, i == 4)); col.addView(space(8))
        }
        scroll.addView(col)
        return scroll
    }
    ...
    private fun settings(): View {
        ...
        ).forEach { (a,b,c) -> col.addView(settingRow(a,b,c)); col.addView(space(10)) }
    ...
    private fun showBreak() {
        val content = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; gravity = Gravity.CENTER_HORIZONTAL; setPadding(dp(18), dp(12), dp(18), dp(8)) }
        content.addView(ImageView(this).apply { setImageResource(R.drawable.cheer_full); scaleType = ImageView.ScaleType.FIT_CENTER }, LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, dp(190)))
        ...
        AlertDialog.Builder(this).setView(content).setNegativeButton("5분 더", null).setPositiveButton("알았어, 쉴게", null).show()
    }
    ...
    private fun metric(title: String, value: String, accent: Boolean): View = LinearLayout(this).apply {
        orientation = LinearLayout.VERTICAL; gravity = Gravity.CENTER_VERTICAL; background = cardBg(); setPadding(dp(15), 0, dp(15), 0)
        addView(label(title, 12, muted, false)); addView(label(value, 20, if (accent) purple else text, true))
    }

    private fun statRow(title: String, value: String, accent: Boolean): View = LinearLayout(this).apply {
        orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL; background = cardBg(); setPadding(dp(18), 0, dp(18), 0)
        addView(label(title, 13, muted, false)); addView(View(this@MainActivity), LinearLayout.LayoutParams(0, 1, 1f)); addView(label(value, 16, if (accent) purple else text, true))
    ...
    private fun settingRow(title: String, subtitle: String, checked: Boolean): View = LinearLayout(this).apply {
        orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL; background = cardBg(); setPadding(dp(18), 0, dp(14), 0)
        val words = LinearLayout(this@MainActivity).apply { orientation = LinearLayout.VERTICAL }
        words.addView(label(title, 15, text, true)); words.addView(label(subtitle, 12, muted, false))
        addView(words, LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f)); addView(Switch(this@MainActivity).apply { isChecked = checked; isEnabled = false })
    ...
    private fun label(value: String, sp: Int, color: Int, bold: Boolean): TextView = TextView(this).apply {
        text = value; textSize = sp.toFloat(); setTextColor(color)
    ...
    private fun rounded(fill: Int, radiusDp: Float, stroke: Int? = null) = GradientDrawable().apply {
        shape = GradientDrawable.RECTANGLE; cornerRadius = dp(radiusDp.toInt()).toFloat(); setColor(fill); if (stroke != null) setStroke(dp(1), stroke)
    }
    ...
    private fun fullWrapMargins(l: Int, t: Int, r: Int, b: Int) = LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT).apply { setMargins(dp(l), dp(t), dp(r), dp(b)) }
}
